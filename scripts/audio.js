...
//   const content = `
//     <div style="padding: 10px; color: white;" id="hs-upload-modal-wrapper">
...
//       <div id="hs-upload-progress-container" style="display: none; margin-top: 10px; background: rgba(0,0,0,0.3); padding: 16px; border-radius: 8px; border: 1px solid #3f3f46;">
//         <div style="font-size: 13px; font-weight: 500; color: #e4e4e7; margin-bottom: 8px; display: flex; justify-content: space-between;">
//           <span id="hs-upload-status-text">Uploading File...</span>
//           <span id="hs-upload-progress-text" style="color: #4ade80;">0%</span>
//         </div>
//         <div id="hs-upload-progress-track" style="width: 100%; height: 8px; background: #27272a; border-radius: 9999px; overflow: hidden; border: 1px solid #18181b;">
//           <div id="hs-upload-progress-bar" style="width: 0%; height: 100%; background: #4ade80; transition: width 0.1s linear;"></div>
//         </div>
//       </div>
//     </div>
//   `;
...
//         const btnUpload = html.closest('.app').find('button[data-button="upload"]');
//         btnUpload.off("click").on("click", async (e) => {
//           e.preventDefault();
          
//           if (!selectedFile) {
//             ui.notifications?.warn("Please select an audio file.");
//             return;
//           }

//           const nameVal = nameInp.value.trim() || selectedFile.name.split(".")[0];

//           html.find("#hs-audio-upload-box").hide();
//           html.find("#hs-upload-progress-container").show();
//           html.closest('.app').find('.dialog-buttons button').prop("disabled", true);
...
//           xhr.open("POST", `${API_BASE_URL}/api/silane_assets/audio/upload`);
//           xhr.setRequestHeader("Authorization", `Bearer ${getToken()}`);

//           xhr.upload.onprogress = (event) => {
//             if (event.lengthComputable) {
//               const percent = Math.round((event.loaded / event.total) * 100);
//               html.find("#hs-upload-progress-bar").css("width", percent + "%");
              
//               if (percent >= 100) {
//                 html.find("#hs-upload-status-text").text("Processing Upload...");
//                 html.find("#hs-upload-progress-text").html('<i class="fa-solid fa-circle-notch fa-spin"></i>');
//                 html.find("#hs-upload-progress-track").hide();
//               } else {
//                 html.find("#hs-upload-progress-text").text(percent + "%");
//               }
//             }
//           };
...
//               } catch (err) {
//                 ui.notifications?.error("Error parsing response data.");
//               }
//             } else {
...
//           xhr.onerror = () => {
//             ui.notifications?.error("Network error during upload.");
//             d.close();
//           };
