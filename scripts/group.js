    ...
    state.owned.forEach((group) => {
      ...
      html += `
        <div class="hs-group-card" style="background: rgba(24, 24, 27, 0.5); border: 1px solid #3f3f46; border-radius: 12px; padding: 16px; cursor: pointer; display: flex; align-items: center; transition: all 0.2s;" data-id="${group.id}">
          <div style="width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 10px; margin-right: 16px; background: rgba(${rgbColor}, 0.1); border: 1px solid rgba(${rgbColor}, 0.3);">
            <i class="fa-solid fa-shield-halved" style="font-size: 22px; color: rgb(${rgbColor});"></i>
          </div>
  ...
  if (state.member.length === 0) {
    ...
    state.member.forEach((group) => {
      ...
      html += `
        <div class="hs-group-card" style="background: rgba(24, 24, 27, 0.5); border: 1px solid #3f3f46; border-radius: 12px; padding: 16px; cursor: pointer; display: flex; align-items: center; transition: all 0.2s;" data-id="${group.id}">
          <div style="width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; border-radius: 10px; margin-right: 16px; background: rgba(${rgbColor}, 0.1); border: 1px solid rgba(${rgbColor}, 0.3);">
            <i class="fa-solid fa-users" style="font-size: 20px; color: rgb(${rgbColor});"></i>
          </div>
          <div style="flex: 1; overflow: hidden;">
            <div style="font-weight: bold; font-size: 15px; color: #f4f4f5; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${group.name}</div>
...
function renderGroupDetails() {
  ...
  let html = `
    <div style="padding: 10px; display:flex; flex-direction:column; height: 100%; overflow-y: auto;">
      ...
      <div style="display: flex; gap: 8px; border-bottom: 1px solid #3f3f46; padding-bottom: 8px; margin-bottom: 16px; flex-shrink: 0; overflow-x: auto; max-width: 100%;">
        ...
        <button class="hs-group-tab-btn" data-tab="journal" style="padding: 6px 12px; border-radius: 6px; font-weight: 600; font-size: 12px; cursor: pointer; border: none; transition: all 0.2s; background: ${state.currentSubTab === 'journal' ? `rgba(${rgbColor}, 0.2)` : 'transparent'}; color: ${state.currentSubTab === 'journal' ? 'white' : '#a1a1aa'}; border: 1px solid ${state.currentSubTab === 'journal' ? `rgba(${rgbColor}, 0.4)` : 'transparent'}; white-space: nowrap;">
          Journals
        </button>
        -->
  ...
  if (state.currentSubTab === "members") {
    ...
    html += `
      <div style="display:flex; justify-content:flex-end; margin-bottom:12px; flex-shrink: 0;">
        <button id="hs-btn-share-resource" style="display: flex; align-items: center; gap: 8px; background: #2563eb; color: white; border: none; border-radius: 8px; padding: 6px 14px; font-size: 13px; font-weight: 600; cursor: pointer; width: auto; white-space: nowrap;">
          <i class="fa-solid fa-share-nodes"></i> Share ${isNpcTab ? "NPC" : "Player Character"}
        </button>
      </div>
      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; padding-bottom: 40px;">
    `;

    const filteredResources = resources.filter(r => r.type === typeKey);
    if (filteredResources.length === 0) {
      ...
      filteredResources.forEach(r => {
        const canRemove = isOwner || String(r.owner_id) === String(state.currentUser.id);
        ...
        html += `
          <div class="hs-resource-card" data-id="${r.resource_id}" data-type="${typeKey}" data-name="${r.name}" data-owner="${r.owner_name}" style="background: rgba(24, 24, 27, 0.4); border: 1px solid rgba(63, 63, 70, 0.8); border-radius: 12px; padding: 12px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; transition: all 0.2s;">
            <div style="display: flex; align-items: center; gap: 12px; min-width: 0;">
              <img src="${imgUrl}" style="width: 36px; height: 36px; border-radius: 6px; object-fit: cover; border: 1px solid #3f3f46; flex-shrink: 0;" />
              <div style="min-width: 0;">
                <div style="font-weight:bold; font-size:14px; color:#f4f4f5; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${r.name}</div>
            ...
            <div style="display: flex; gap: 6px; align-items: center; flex-shrink: 0;">
              ${!hasVtt ? `<i class="fa-solid fa-circle-question" style="color: #eab308; font-size: 13px;" title="Not present in VTT (Click for details)"></i>` : ""}
              ${canRemove ? `<button class="hs-remove-resource" data-id="${r.resource_id}" title="Remove Share" style="width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; border-radius: 6px; background: rgba(0,0,0,0.3); border: 1px solid rgba(239, 68, 68, 0.2); color: #f87171; cursor: pointer;"><i class="fa-solid fa-trash" style="font-size:11px;"></i></button>` : ""}
            </div>
          </div>
        `;
      });
    ...
    html += `</div>`;
  } else if (state.currentSubTab === "journal") {
    html += `
      <div style="display:flex; justify-content:flex-end; margin-bottom:12px; flex-shrink: 0;">
        <button id="hs-btn-share-journal" style="display: flex; align-items: center; gap: 8px; background: #2563eb; color: white; border: none; border-radius: 8px; padding: 6px 14px; font-size: 13px; font-weight: 600; cursor: pointer; width: auto; white-space: nowrap;">
          <i class="fa-solid fa-share-nodes"></i> Share Journal
        </button>
      </div>
      <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 16px; padding-bottom: 40px;">
    `;

    const journalResources = resources.filter(r => r.type === "journal");
    if (journalResources.length === 0) {
      html += `
        <div style="grid-column: 1/-1; display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px 0; border: 2px dashed #3f3f46; border-radius: 12px; color: #71717a;">
          <i class="fa-solid fa-book fa-2x" style="margin-bottom: 12px; opacity: 0.5;"></i>
          <div style="font-size: 14px; font-weight: 500; color: #a1a1aa;">No shared journals yet.</div>
        </div>
      `;
    } else {
      journalResources.forEach(r => {
        const canRemove = isOwner || String(r.owner_id) === String(state.currentUser.id);
        const hasVtt = game.journal.get(r.resource_id);

        html += `
          <div class="hs-resource-card" data-id="${r.resource_id}" data-type="journal" data-name="${r.name}" data-owner="${r.owner_name}" style="background: rgba(24, 24, 27, 0.4); border: 1px solid rgba(63, 63, 70, 0.8); border-radius: 12px; padding: 12px; display: flex; align-items: center; justify-content: space-between; cursor: pointer; transition: all 0.2s;">
            <div style="display: flex; align-items: center; gap: 12px; min-width: 0;">
              <div style="width: 36px; height: 36px; border-radius: 6px; background: rgba(99, 102, 241, 0.1); border: 1px solid rgba(99, 102, 241, 0.3); display: flex; align-items: center; justify-content: center; flex-shrink: 0;">
                <i class="fa-solid fa-file-lines" style="color: #818cf8; font-size: 16px;"></i>
              </div>
              <div style="min-width: 0;">
                <div style="font-weight:bold; font-size:14px; color:#f4f4f5; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${r.name}</div>
                <div style="font-size:11px; color:#71717a; margin-top:2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">Shared by: ${r.owner_name}</div>
          ...
          <div class="hs-mission-card" data-id="${m.id}" style="background: rgba(24, 24, 27, 0.4); border: 1px solid rgba(63, 63, 70, 0.8); border-radius: 12px; overflow: hidden; display: flex; flex-direction: column; cursor: pointer; transition: all 0.2s;">
            ...
            <div style="padding: 16px; display: flex; flex-direction: column; gap: 10px; flex: 1; justify-content: space-between;">
              <div>
                <div style="display:flex; justify-content:space-between; align-items:flex-start; gap: 8px;">
                  <div style="display: flex; gap: 6px; align-items: center; flex-wrap: wrap;">
                    <span style="font-size: 9px; font-weight: bold; background: rgba(255,255,255,0.05); color: #d4d4d8; border: 1px solid rgba(255,255,255,0.1); padding: 1px 6px; border-radius: 4px; text-transform: uppercase;">${m.type}</span>
              ...
              <div style="display: flex; flex-direction: column; gap: 6px; margin-top: auto;">
                ${m.reward ? `<div style="font-size: 12px; color: #fbbf24; font-weight: 500; display:flex; align-items:center; gap:6px;"><i class="fa-solid fa-coins"></i> Reward: ${m.reward}</div>` : ""}
                ${Array.isArray(m.objectives) && m.objectives.length > 0 ? `
                  <div style="font-size: 11px; color: #a1a1aa; display: flex; align-items: center; gap: 6px;">
                    ...
                    <span>Objectives: ${m.objectives.filter(o => o.completed || o.current >= o.amount).length} / ${m.objectives.length}</span>
                  </div>
                ` : ""}
  ...
  if (state.currentSubTab === "player" || state.currentSubTab === "npc") {
    const isNpcTab = state.currentSubTab === "npc";
    const btnShare = parentContainer.querySelector("#hs-btn-share-resource");
    if (btnShare) btnShare.onclick = () => showShareResourceModal(group.id, isNpcTab);

    parentContainer.querySelectorAll(".hs-resource-card").forEach((card) => {
      card.onclick = (e) => {
        if (e.target.closest(".hs-remove-resource")) return;
  ...
  // Journals events
  if (state.currentSubTab === "journal") {
    const btnShare = parentContainer.querySelector("#hs-btn-share-journal");
    if (btnShare) btnShare.onclick = () => showShareJournalModal(group.id);
    ...
    parentContainer.querySelectorAll(".hs-resource-card").forEach((card) => {
      card.onclick = (e) => {
        ...
        const name = card.dataset.name;
        const owner = card.dataset.owner;
        openResourceDetails(resourceId, "journal", name, owner);
      };
    });
    ...
    parentContainer.querySelectorAll(".hs-remove-resource").forEach((btn) => {
      btn.onclick = async (e) => {
        e.stopPropagation();
        if (!window.confirm("Remove this shared journal from the group?")) return;
        const resourceId = btn.dataset.id;
        ui.notifications?.info("Removing shared journal...");
        try {
          const response = await fetch(`${API_BASE_URL}/api/groups/${group.id}/resources/${resourceId}`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${getToken()}` }
          ...
          if (response.ok) {
            ui.notifications?.info("Shared journal removed.");
            await fetchGroupsData();
          } else {
            const err = await response.json();
            ui.notifications?.error(err.message || "Failed to remove shared journal.");
          }
        } catch (e) {
          ui.notifications?.error("Failed to remove shared journal.");
        }
      };
    });
  }
  ...
  if (state.currentSubTab === "missions") {
    ...
    if (btnCreateMission) btnCreateMission.onclick = () => showCreateOrEditMissionModal(group.id);
    ...
    parentContainer.querySelectorAll(".hs-edit-mission").forEach((btn) => {
      btn.onclick = (e) => {
        ...
        const mission = missions.find(m => m.id === missionId);
        if (mission) showCreateOrEditMissionModal(group.id, mission);
      };
    });
...
function showChangeRoleModal(groupId, memberUserId, currentRole, groupRoles) {
  ...
  const content = `
    <div style="padding: 10px; color: white;">
      ...
      <select id="hs-change-member-role" class="silane-input" style="width:100%; border-radius:6px; padding:8px; background: #27272a; color: #ffffff; border: 1px solid #52525b; font-size: 14px;">
        ${optionsHtml}
      </select>
    </div>
  `;
...
function showShareResourceModal(groupId, isNpc = false) {
  ...
  const content = `
    <div style="padding: 10px; color: white;">
      ...
      <select id="hs-share-actor-select" class="silane-input" style="width:100%; border-radius:6px; padding:8px; background: rgba(0,0,0,0.3); color: #ffffff; border: 1px solid #52525b;">
        ${optionsHtml}
      </select>
    </div>
  `;
...
function showShareJournalModal(groupId) {
  const journals = game.journal.map(j => ({ id: j.id, name: j.name }));

  if (journals.length === 0) {
    return ui.notifications?.warn("No journals found in your sidebar to share.");
  }

  const optionsHtml = journals.map(j => `
    <option value="${j.id}" data-name="${j.name}">
      ${j.name}
    </option>
  `).join("");
  ...
  const content = `
    <div style="padding: 10px; color: white;">
      <label style="display:block; margin-bottom:8px; color:#e4e4e7; font-weight: 500;">Select Journal to Share</label>
      <select id="hs-share-journal-select" class="silane-input" style="width:100%; border-radius:6px; padding:8px; background: rgba(0,0,0,0.3); color: #ffffff; border: 1px solid #52525b;">
        ${optionsHtml}
      </select>
    </div>
  `;
  ...
  new Dialog(
    {
      ...
      buttons: {
        share: {
          label: "Share",
          callback: async (html) => {
            const selectElem = html.find("#hs-share-journal-select")[0];
            const selectedOpt = selectElem.options[selectElem.selectedIndex];
            
            const resource_id = selectedOpt.value;
            const name = selectedOpt.getAttribute("data-name");

            ui.notifications?.info("Sharing journal with group...");
            try {
              const res = await fetch(`${API_BASE_URL}/api/groups/${groupId}/resources`, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({ type: "journal", resource_id, name }),
              });
              if (res.ok) {
                ui.notifications?.info("Journal shared successfully!");
                await fetchGroupsData();
              } else {
                const err = await res.json();
                ui.notifications?.error(err.message || "Failed to share journal.");
              }
            } catch (e) {
              ui.notifications?.error("Server error.");
            }
...
function openResourceDetails(resourceId, type, name, ownerName) {
  if (type === "player" || type === "npc") {
    ...
    const journal = game.journal.get(resourceId);
    if (journal) {
  ...
  const content = `
    <div style="padding: 15px; color: white; display: flex; flex-direction: column; gap: 12px; font-family: sans-serif;">
      <div style="display: flex; align-items: center; gap: 12px;">
        <div style="width: 48px; height: 48px; border-radius: 8px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.15); display: flex; align-items: center; justify-content: center;">
          <i class="fa-solid ${type === 'journal' ? 'fa-file-lines' : type === 'npc' ? 'fa-user-ninja' : 'fa-user-shield'}" style="font-size: 20px; color: #a1a1aa;"></i>
        </div>
        <div>
          <h3 style="margin: 0; font-size: 16px; color: white;">${name}</h3>
          <span style="font-size: 11px; color: #a1a1aa;">Type: ${type.toUpperCase()}</span>
        </div>
      </div>
      <hr style="border: 0; border-top: 1px solid #3f3f46; margin: 5px 0;" />
      <div style="font-size: 13px; line-height: 1.5;">
        <p style="margin: 0 0 6px 0;"><strong>Shared by:</strong> ${ownerName}</p>
        <p style="margin: 0; color: #fbbf24; background: rgba(251, 191, 36, 0.1); border: 1px solid rgba(251, 191, 36, 0.2); padding: 8px; border-radius: 6px; display: flex; align-items: center; gap: 8px;">
          <i class="fa-solid fa-triangle-exclamation"></i>
          <span>This sheet is not present in your local VTT world, or you do not have permission to view it.</span>
        </p>
      </div>
    </div>
  `;
  ...
  new Dialog({
    ...
    default: "ok",
    render: applyDarkThemeToDialog
  }, { width: 360, classes: ["dialog", "silane-custom-dialog"] }).render(true);
}

function showCreateOrEditMissionModal(groupId, mission = null) {
  ...
  const missionReward = isEdit ? mission.reward : "";
  ...
  const content = `
    <div style="padding: 10px; color: white; max-height: 70vh; overflow-y: auto; font-family: sans-serif;">
      ...
      <div style="margin-bottom: 12px;">
        ...
        <select id="hs-mission-type" class="silane-input" style="width:100%; border-radius:6px; padding:8px; background: rgba(0,0,0,0.3); color: #ffffff; border: 1px solid #52525b;">
          <option value="main" ${missionType === 'main' ? 'selected' : ''}>Main Quest</option>
          <option value="side" ${missionType === 'side' ? 'selected' : ''}>Side Quest</option>
        </select>
      </div>
      ${isEdit ? `
      <div style="margin-bottom: 12px;">
        <label style="display:block; margin-bottom:5px; color:#e4e4e7; font-weight: 500;">Mission Status</label>
        <select id="hs-mission-status" class="silane-input" style="width:100%; border-radius:6px; padding:8px; background: #27272a; color: #ffffff; border: 1px solid #52525b; font-size: 14px;">
      ...
      <div style="margin-bottom: 12px;">
        <label style="display:block; margin-bottom:5px; color:#e4e4e7; font-weight: 500;">Reward (Optional)</label>
        <input type="text" id="hs-mission-reward" class="silane-input" style="width:100%; border-radius:6px; padding:8px; background: rgba(0,0,0,0.3); color: #ffffff; border: 1px solid #52525b;" placeholder="e.g. 500 gold, magical items" value="${missionReward}" />
      </div>
      <div style="margin-bottom: 12px;">
        <label style="display:block; margin-bottom:5px; color:#e4e4e7; font-weight: 500;">Objectives</label>
        <div id="hs-objectives-list" style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 8px;">
          <!-- Rows will be injected here -->
        </div>
        <button type="button" id="hs-add-objective-btn" style="background: rgba(255,255,255,0.05); border: 1px solid #3f3f46; color: white; border-radius: 6px; padding: 6px 12px; font-size: 11px; cursor: pointer; width: auto; font-weight: 600;">
  ...
  new Dialog(
    {
      ...
      buttons: {
        save: {
          ...
          callback: async (html) => {
            ...
            const reward = html.find("#hs-mission-reward").val().trim();
            ...
            const objectives = [];
            html.find(".hs-objective-row").each((i, el) => {
              const row = $(el);
              const type = row.find(".obj-type").val();
              const target = row.find(".obj-target").val().trim();
              const amount = parseInt(row.find(".obj-amount").val()) || 1;
              const current = parseInt(row.find(".obj-current").val()) || 0;
              if (target) {
                objectives.push({
                  type,
                  target,
            ...
            try {
              const res = await fetch(url, {
                ...
                body: JSON.stringify({ title, type, status, description, reward, objectives, image, required_level, notes, player_notes }),
              });
              if (res.ok) {
      ...
      render: (html) => {
        ...
        const list = html.find("#hs-objectives-list");
        const btn = html.find("#hs-add-objective-btn");

        const addObjectiveRow = (obj = { type: "defeat", target: "", amount: 1, current: 0 }) => {
          const row = $(`
            <div class="hs-objective-row" style="display: flex; gap: 6px; align-items: center;">
              <select class="obj-type" style="flex: 2; border-radius:6px; padding:6px; background: rgba(0,0,0,0.4); color: white; border: 1px solid #52525b;">
              ...
              <input type="text" class="obj-target" value="${obj.target}" style="flex: 4; border-radius:6px; padding:6px; background: rgba(0,0,0,0.4); color: white; border: 1px solid #52525b;" placeholder="Target name..." />
              <input type="number" class="obj-amount" value="${obj.amount}" style="width: 55px; border-radius:6px; padding:6px; background: rgba(0,0,0,0.4); color: white; border: 1px solid #52525b;" min="1" placeholder="Qty" />
              <input type="number" class="obj-current" value="${obj.current || 0}" style="width: 55px; border-radius:6px; padding:6px; background: rgba(0,0,0,0.4); color: white; border: 1px solid #52525b;" min="0" placeholder="Done" />
              <button type="button" class="hs-remove-obj-btn" style="width:30px; height:30px; border-radius:6px; background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); color: #f87171; cursor: pointer; display: flex; align-items: center; justify-content: center;">
                <i class="fa-solid fa-xmark"></i>
              </button>
            </div>
          `);
          row.find(".hs-remove-obj-btn").click(() => row.remove());
          list.append(row);
        };

        if (isEdit && Array.isArray(mission.objectives)) {
          mission.objectives.forEach(obj => addObjectiveRow(obj));
        }

        btn.click(() => addObjectiveRow());
      }
    },
    { width: 480, classes: ["dialog", "silane-custom-dialog"] }
  ).render(true);
}
      ...
      <div style="margin-top: 15px;">
        ...
        <div style="display: flex; flex-direction: column; gap: 8px;">
          ${objectives.map((obj) => {
            ...
            return `
              <div style="display: flex; align-items: center; gap: 10px; background: rgba(0,0,0,0.2); border: 1px solid ${isDone ? 'rgba(52, 211, 153, 0.2)' : '#3f3f46'}; padding: 8px 12px; border-radius: 6px;">
                ...
                <div style="flex: 1; min-width: 0;">
                  ...
                  <div style="display: flex; align-items: center; gap: 8px; margin-top: 4px;">
                    ...
                    <span style="font-size: 11px; color: #a1a1aa; font-family: monospace; font-weight: 600; shrink: 0;">${obj.current || 0} / ${obj.amount || 1}</span>
                  </div>
                </div>
  ...
  const content = `
    <div style="padding: 15px; color: white; display: flex; flex-direction: column; gap: 12px; font-family: sans-serif; max-height: 70vh; overflow-y: auto;">
      ...
      <div style="font-size: 13px; line-height: 1.5; color: #d4d4d8; display: flex; flex-direction: column; gap: 10px;">
        ...
        ${mission.reward ? `
          <div style="margin-top: 6px; padding: 10px; border-radius: 8px; border: 1px solid rgba(251, 191, 36, 0.3); background: rgba(251, 191, 36, 0.05); display: flex; align-items: center; gap: 8px; color: #fbbf24; font-weight: 600;">
  ...
  const content = `
    <div style="padding: 10px; color: white;">
      ...
      <div style="margin-bottom: 12px;">
        ...
        <input type="color" id="hs-edit-group-color" style="width:100%; height:40px; border-radius:6px; border: 1px solid #52525b; background: transparent; cursor: pointer;" value="${group.color || '#6366f1'}" />
      </div>
        ...
        callback: async (html) => {
          ...
          try {
            const res = await fetch(`${API_BASE_URL}/api/groups/${group.id}`, {
              ...
              body: JSON.stringify({ name, description, color, ...(password ? { password } : {}) }),
            });
            if (res.ok) {
